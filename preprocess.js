...
metadata.forEach((bc) => {
  ...
  result[bc.name].monthly_weekday = [];
  result[bc.name].monthly_weekend = [];
  result[bc.name].monthly = months.map((monthStr, i) => {
    ...
    ['weekday', 'weekend'].forEach((dayType) => {
      ...
      const dayTypeData = data.filter(r => acceptableDays.has(dayOfWeek(r)));
      const dates = dayTypeData.map(r => r.date.slice(0,10));
      const uniqueDays = new Set(dates);
      const tmp = (sumField(dayTypeData, bc.dirs[0]) + sumField(dayTypeData, bc.dirs[1])) / uniqueDays.size;
      result[bc.name][`monthly_${dayType}`].push(tmp);
    });
